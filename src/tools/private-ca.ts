...
export function registerPrivateCaTools(server: McpServer, client: NcloudClient): void {
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const result = await client.requestRaw("POST", `/api/v1/ca/${params.caTag}/activate`, undefined, {
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const result = await client.requestRaw("POST", `/api/v1/ca/${params.caTag}/sub/sign`, undefined, {
          period: params.period,
