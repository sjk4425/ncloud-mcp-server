...
> Sub Account (IAM) audit release. The 15 Sub Account tools were compared one-by-one against all 79 operations in the [official Sub Account API docs](https://api.ncloud-docs.com/docs/management-subaccount); the comparison turned up **two tools whose request URI made success impossible** and **two parameters that do not exist in the API at all**, alongside the two feature gaps already filed as `MCP-BUG-REPORT` #3 and #4. Both gaps are now closed (11 new tools: 4 access-key, 7 policy). Backward-compatible — no tool was removed and no parameter was dropped from a schema; the two non-existent parameters are still accepted and now explicitly reported as ignored. Specs confirmed against the official docs; behavior covered by mocked unit tests (**not yet exercised against the live API**).
