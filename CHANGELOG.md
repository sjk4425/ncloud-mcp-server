...
> Sub Account (IAM) and Data Catalog audit release. Both services' tools were compared one-by-one against every operation in their official API docs — 79 for [Sub Account](https://api.ncloud-docs.com/docs/management-subaccount), 27 for [Data Catalog](https://api.ncloud-docs.com/docs/analytics-datacatalog). The comparison turned up **five tools whose request URI made success impossible**, **two parameters that do not exist in the API at all**, and a **path-encoding defect** affecting 15 tools, alongside the three feature gaps already filed as `MCP-BUG-REPORT` #3, #4 and #5. All three gaps are now closed (17 new tools: 4 access-key, 7 policy, 6 Data Catalog write). Backward-compatible — no tool was removed and no parameter was dropped from a schema; the two non-existent parameters are still accepted and now explicitly reported as ignored. Specs confirmed against the official docs; behavior covered by mocked unit tests (**not yet exercised against the live API**).
